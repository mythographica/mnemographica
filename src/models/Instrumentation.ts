...
export const Instrumentation = define('Instrumentation', class {
	...
	// Why there is no creation graph: the Diamonds pane must say WHICH —
	// 'missing' (no instrumentation.json at all), 'stale' (the file exists
	// but is older than definitions.json, so the guard skipped it), 'v1'
	// (loaded fine, but predates tactica v2 — or its graph section was
	// malformed)
